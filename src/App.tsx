...
export default function PokaListApp() {
  ...
  let srcParam = srcRaw;
  try {
    srcParam = decodeURIComponent(srcRaw);
  } catch {}
  ...
  const shareKey = useMemo(
    () => `pokaShareChecks:${srcParam || "local"}`,
    [srcParam]
  );
  const [myChecks, setMyChecks] = useState<Record<string, boolean>>({});
  useEffect(() => {
    try {
      const v = localStorage.getItem(shareKey);
      if (v) setMyChecks(JSON.parse(v));
    } catch {}
  }, [shareKey]);
  useEffect(() => {
    try {
      localStorage.setItem(shareKey, JSON.stringify(myChecks));
    } catch {}
  }, [shareKey, myChecks]);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!aborted)
          setLoadError(
            "src JSON을 불러오지 못했습니다. URL과 CORS 설정(GitHub Pages 권장)을 확인해주세요."
          );
      }
    })();
    return () => {
  ...
  const filtered = useMemo(() => {
    ...
    return items.filter((c) => {
      ...
      const hay = `${c.title} ${c.event} ${c.vendor} ${c.notes} ${c.purchaseDate}`
        .toLowerCase()
        .replaceAll("\n", " ");
  ...
  const eventOptions = useMemo(
    () => ["전체", ...Array.from(new Set(items.map((c) => c.event).filter(Boolean)))],
    [items]
  );
  const yearOptions = useMemo(
    () =>
      ["전체", ...Array.from(new Set(items.map((c) => c.year).filter(Boolean))).sort((a: any, b: any) => Number(a) - Number(b))],
    [items]
  );
  ...
  async function ghApi(path: string, contentB64: string, message: string) {
    ...
    if (!res.ok) {
      ...
      throw new Error(`GitHub API ${res.status}${d ? `: ${d}` : ""}`);
    }
    return res.json();
  }
  ...
  async function adminCommitCatalog() {
    try {
      const payload = {
        v: 1,
        title: "BOGUMMY",
        items: items.map(({ __i, __idx, __file, ...rest }) => rest),
      };
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900">
          ...
          <div className="flex items-center gap-2 ml-auto flex-nowrap overflow-x-auto">
            <button
              ...
              className={`p-2 rounded-lg ${
                view === "gallery" ? "bg-slate-200" : "hover:bg-slate-100"
              }`}
              title="갤러리 보기"
            >
              <GridIcon size={18} />
            ...
            <button
              ...
              className={`p-2 rounded-lg ${
                view === "table" ? "bg-slate-200" : "hover:bg-slate-100"
              }`}
              title="표 보기"
            >
              <List size={18} />
            ...
            {/* 공유 모드 안내 텍스트 */}
            {shareMode && (
              <div className="text-sm text-slate-600 ml-2 truncate">
                보유 체크 현황은 이 브라우저에만 저장됩니다.
              </div>
            )}
            ...
            <div className="admin-only flex items-center gap-2 flex-nowrap">
              ...
              <label className="flex items-center gap-1 bg-slate-700 text-white px-3 py-2 rounded-lg text-sm hover:bg-slate-800 cursor-pointer">
                ...
                <input
                  ...
                  multiple
                  className="hidden"
                  onChange={(e) => handleMultiAdd(e.target.files)}
                />
              </label>
              <button
                onClick={() => createSrcShareLink()}
                className="flex items-center gap-1 bg-amber-500 text-white px-3 py-2 rounded-lg text-sm hover:bg-amber-600"
              >
                <LinkIcon size={14} /> JSON 주소로 공유(src)
              </button>
              <button
                onClick={() => setAdminOpen((v) => !v)}
                className="flex items-center gap-1 bg-purple-600 text-white px-3 py-2 rounded-lg text-sm hover:bg-purple-700"
              >
                관리자 패널 {adminOpen ? "닫기" : "열기"}
              </button>
            </div>
          </div>
        </div>
              ...
              <div className="flex flex-wrap items-center gap-2">
                <label className="px-3 py-2 rounded-xl bg-slate-200 hover:bg-slate-300 inline-flex items-center gap-2 cursor-pointer">
                  ...
                  <input
                    ...
                    multiple
                    className="hidden"
                    onChange={(e) => adminUploadImages(e.target.files)}
                  />
                </label>
                <button
                  onClick={adminCommitCatalog}
                  className="px-3 py-2 rounded-xl bg-emerald-600 text-white hover:bg-emerald-700"
                >
                  catalog.json 커밋
                </button>
                <div className="text-xs text-slate-600">
                  커밋 후 1~2분 후 공유 링크에 반영됩니다.
                </div>
              </div>
            </div>
          </div>
      ...
      <main className="mx-auto max-w-6xl px-4 py-8">
        ...
        {orderedYearKeys.map((year) => (
          <section key={year} className="mb-8">
            <h2 className="text-lg font-semibold mb-3 border-b border-slate-300 pb-1">
              {year}
            </h2>
            <div
            ...
            >
              {grouped[year]?.map((card, i) => {
                ...
                return (
                  <article
                    key={card.id || i}
                    className="bg-white shadow rounded-xl p-2 border border-slate-200"
                  >
                    <div className="relative">
                      <div
                        className="w-full rounded-xl border border-slate-200 overflow-hidden aspect-[2/3] bg-white cursor-pointer"
                      ...
                      <div className="admin-only absolute top-2 right-2 flex gap-1">
                        <button
                          className="p-1.5 rounded bg-white/90 hover:bg-white shadow"
                          title="수정"
                          onClick={(e) => {
                            e.stopPropagation();
                            setDetail(card);
      ...
      {detail && (
        <div
          className="fixed inset-0 z-[120] bg-black/50 backdrop-blur-sm grid place-items-center p-3"
          onClick={() => setDetail(null)}
        >
          <div
            className="bg-white rounded-2xl w-[92vw] max-w-[300px] md:max-w-[360px] shadow-xl"
            onClick={(e) => e.stopPropagation()}
          >
            ...
            <div className="p-3 space-y-3">
              <div
                className={`relative rounded-xl border overflow-hidden aspect-[2/3] bg-slate-50 grid place-items-center ${
                  shareMode ? "cursor-default" : "cursor-pointer"
                }`}
                onClick={
                  shareMode ? undefined : () => detailFileRef.current?.click()
                }
              >
                {detail.imageUrl ? (
                  <img
                ...
                <input
                  ...
                  type="file"
                  accept="image/*"
                  className="admin-only hidden"
                  onChange={(e) => onDetailImageSelect(e.target.files)}
                />
              </div>
              ...
              <div className="admin-only grid grid-cols-2 gap-2 text-sm">
                ...
                <label>
                  ...
                  <input
                    ...
                    value={detail.event || ""}
                    onChange={(e) =>
                      setDetail({ ...detail, event: e.target.value })
                    }
                  />
                </label>
                <label>
                  ...
                  <input
                    ...
                    value={detail.vendor || ""}
                    onChange={(e) =>
                      setDetail({ ...detail, vendor: e.target.value })
                    }
                  />
                </label>
                <label>
              ...
              <div className="text-sm space-y-1">
                <div>
                  <b>제목</b> {detail.title || "(제목 없음)"}
                </div>
                <div>
                  <b>구매 날짜</b> {detail.purchaseDate || "-"}
                </div>
                <div>
                  <b>이벤트</b> {detail.event || "-"}
                </div>
                <div>
                  <b>구매처</b> {detail.vendor || "-"}
                </div>
                <div>
                  <b>연도</b> {detail.year || "-"}
                </div>
